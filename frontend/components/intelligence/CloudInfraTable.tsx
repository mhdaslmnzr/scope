...
const mockData = [
  { id: 1, service: 'AWS S3', issue: 'Public Bucket', severity: 'High', lastChecked: '2024-06-10' },
  { id: 2, service: 'Azure VM', issue: 'Unpatched OS', severity: 'Medium', lastChecked: '2024-06-09' },
  { id: 3, service: 'GCP Storage', issue: 'Weak Permissions', severity: 'Low', lastChecked: '2024-06-08' },
];

export default function CloudInfraTable() {
  const [search, setSearch] = useState('');
  const [severityFilter, setSeverityFilter] = useState('');

  const filtered = mockData.filter(row =>
    (!search || row.service.toLowerCase().includes(search.toLowerCase())) &&
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Cloud & Infra</h1>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {filtered.map(row => (
              <tr key={row.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{row.service}</td>
