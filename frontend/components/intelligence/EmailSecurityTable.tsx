...
const mockData = [
  { id: 1, domain: 'acme.com', issue: 'No DMARC', severity: 'High', lastChecked: '2024-06-10' },
  { id: 2, domain: 'vpn.acme.com', issue: 'Weak SPF', severity: 'Medium', lastChecked: '2024-06-09' },
  { id: 3, domain: 'mail.acme.com', issue: 'Open Relay', severity: 'Low', lastChecked: '2024-06-08' },
];

export default function EmailSecurityTable() {
  const [search, setSearch] = useState('');
  const [severityFilter, setSeverityFilter] = useState('');

  const filtered = mockData.filter(row =>
    (!search || row.domain.toLowerCase().includes(search.toLowerCase())) &&
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Email Security</h1>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {filtered.map(row => (
              <tr key={row.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{row.domain}</td>
