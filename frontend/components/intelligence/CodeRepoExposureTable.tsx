...
const mockData = [
  { id: 1, repo: 'github.com/acme/app', issue: 'Sensitive Data', severity: 'High', lastChecked: '2024-06-10' },
  { id: 2, repo: 'gitlab.com/acme/api', issue: 'Public Repo', severity: 'Medium', lastChecked: '2024-06-09' },
  { id: 3, repo: 'bitbucket.org/acme/infra', issue: 'Weak Secrets', severity: 'Low', lastChecked: '2024-06-08' },
];

export default function CodeRepoExposureTable() {
  const [search, setSearch] = useState('');
  const [severityFilter, setSeverityFilter] = useState('');

  const filtered = mockData.filter(row =>
    (!search || row.repo.toLowerCase().includes(search.toLowerCase())) &&
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Code Repo Exposure</h1>
      </div>
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-200">
        <table className="min-w-full divide-y divide-gray-100">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Repo</th>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {filtered.map(row => (
              <tr key={row.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{row.repo}</td>
